...
const Backspace = styled(NButton)`
  color: ${props => props.theme.subHeader.primaryColor};
  visibility: ${props => (props.value.length ? 'visible' : props.theme.subHeader.emptyVisibility)};
`;
...
const DisplayWrapper = ({ value, backspace, longPressBackspace }) => {
  ...
  return (
    <Wrapper>
      ...
      <Backspace {...backspaceLongPress} onClick={backspace} value={value}>
        <IconBackspace />
      </Backspace>
    </Wrapper>
  );
